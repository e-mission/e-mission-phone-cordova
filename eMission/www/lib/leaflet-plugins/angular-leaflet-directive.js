    ...
    return {
        ...
        link: function(scope, element, attrs, ctrl) {
            ...
            if (!isDefined(attrs.tiles) && (!isDefined(attrs.layers))) {
                ...
                tileLayerObj.name = "tileLayer";
                tileLayerObj.addTo(map);
                /*
                tileLayerObj.on('load', function() {
                    console.log("Finished loading tile Layer");
                    tileLayerObj.setOpacity(0.1);
                    tileLayerObj.setZIndex(-100);
                    tileLayerObj.bringToBack();
                    // console.log("geojson layer has name "+leafletData.getGeoJSON().name);
                    // leafletData.getGeoJSON().bringToFront();
                    map.eachLayer(function(layer) {
                        console.log("******");
                        console.log("map has layer "+layer.name);
                    });
                });
                */
...
/*
                    L.magnifyingGlass({
                        ...
                        fixedPosition: true,
                        latLng: reverseAndReturn(geojson.data.features[0].geometry.coordinates)
                    }).addTo(map);
